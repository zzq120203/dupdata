...
public class LogicSyntaxTree {
            ...
            if (operatorPriority.containsKey(charStr) || ")".equals(charStr)) {
                ...
                if ("(".equals(charStr) || ")".equals(charStr)) {
                    ...
                    if (stack.empty() || operatorPriority.get(stack.peek()) <= operatorPriority.get(charStr)) {
                        ...
                        while (!stack.empty()) {
                            if (operatorPriority.get(stack.peek()) >= operatorPriority.get(charStr) && !stack.peek().equals("(")) {
                                list.add(stack.pop());
    ...
    public static void main(String args[]) throws Exception {
        ...
        String[] array = new String[] { "习近平AND特朗普", "银行AND(账号OR帐号)", "淘宝ANDNOT手机淘宝", "朝鲜AND(导弹OR核)" };
        ...
        LogicSyntaxTree ttclone=ttt.cloneObject();
    ...
    public LogicSyntaxTree cloneObject() {
        LogicSyntaxTree ret = new LogicSyntaxTree();
        if(root!=null){
            ret.root=recursiveClone(root,ret.root);
        }
        return ret;
    }

    private LogicOperatorNode recursiveClone(LogicOperatorNode oriNode, LogicOperatorNode newNode) {
        if (newNode == null) {
            newNode = new LogicOperatorNode();
        }
        switch (oriNode.opt) {
            case LEAF:
                newNode.opt = LOGOPT.LEAF;
                newNode.str = new String(oriNode.str);
                break;
            case NOT:
                newNode.opt=LOGOPT.NOT;
                break;
            case AND:
                newNode.opt=LOGOPT.AND;
                break;
            case OR:
                newNode.opt=LOGOPT.OR;
                break;
        }

        if(oriNode.leftOperand!=null){
            newNode.leftOperand=recursiveClone(oriNode.leftOperand,newNode.leftOperand);
        }
        if(oriNode.ritghOperand!=null){
            newNode.ritghOperand=recursiveClone(oriNode.ritghOperand,newNode.ritghOperand);
        }
        return newNode;
    }
}
